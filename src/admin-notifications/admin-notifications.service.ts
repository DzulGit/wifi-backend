  ...
  async processUserRequest(
    ...
    if (!this.isUserRequestNotification(notif.title)) {
      throw new BadRequestException(
        'Notifikasi ini bukan permintaan pelanggan',
      );
    }
    ...
    if (!userId)
      throw new BadRequestException('Metadata userId tidak ditemukan');
    ...
    if (decision === 'APPROVED') {
      if (notif.title.includes('Ganti Paket')) {
        ...
        if (!newPackageId)
          throw new BadRequestException('ID paket baru tidak ditemukan');
        ...
        if (!newAddress?.trim())
          throw new BadRequestException('Alamat baru tidak ditemukan');
        await this.prisma.user.update({
          where: { id: userId },
