...
export class AuthService {
  ...
  async sendOtp(email: string) {
    ...
    if (!user) {
      return {
        message: 'Jika email terdaftar, OTP akan dikirim dalam beberapa saat',
      };
    }
  ...
  async verifyOtp(email: string, code: string) {
    ...
    if (!otp)
      throw new BadRequestException('OTP tidak valid atau sudah expired');
  ...
  async loginWithPassword(email: string, password: string, ip?: string) {
    ...
    if (!user || !user.password) {
      ...
      await bcrypt.compare(
        password,
        '$2b$10$dummyhashfordummypurposesonly12345678',
      );
  ...
  async activateAccount(token: string, password: string) {
    ...
    if (!user)
      throw new BadRequestException(
        'Link aktivasi tidak valid atau sudah expired',
      );
  ...
  async changePassword(
    ...
    if (!user.password)
      throw new BadRequestException('User belum mengatur password');
